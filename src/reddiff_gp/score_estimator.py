...
class  ScoreEstimator(nn.Module):
    def __init__(self, target_log_prob, target_log_prob_and_grad, n_chains:int =16, 
        ...
        # self.diffusion = diffusion # Define the noise schedule of the diffusion model
    ...
    def forward(self, xt, y, alpha_t) -> torch.Tensor:
        ...
        et, _ = self.score_estimation(alpha_t=alpha_t, x=xt, 
    ...
    # def __call__(self, xt, y, t) -> torch.Tensor:
    #     # y: the observation trajectory
    #     # Returns both the estimated score function (noise value) and the predicted x_0
    #     alpha_t = self.diffusion.alpha_t(t).view(-1, 1)
    #     con_target_log_prob = self.target_log_prob(y)
    #     con_target_log_prob_and_grad = self.target_log_prob_and_grad(y)
    #     step_size = torch.tensor(1.0 - torch.exp(-2.0 * self.T)) / 2.0
    #     et = self.score_estimation(t=t, T=self.T, x=xt, 
    #                                step_size=step_size, n_is_samples=self.n_is_samples, n_mcmc_steps=self.n_mcmc_steps, n_chains=self.n_chains,
    #                                target_log_prob=con_target_log_prob, target_log_prob_and_grad=con_target_log_prob_and_grad,)
    #     x0_pred = (xt - et * (1 - alpha_t).sqrt()) / alpha_t.sqrt()
    #     return et, x0_pred
          
    def posterior_log_prob_and_grad(self, y, xt, alpha_t, target_log_prob_and_grad):
        """Compute the posterior distribution of RDMC and its gradient

            q_t(y|x) = pi(x) N(exp(T-t)*x, [(1 - exp(-2*(T-t))) / exp(-2*(T-t))] I)
            q_t(y|x) = pi(x) N(alpha_t*x, [1 - alpha_t^2] I)
        Args:
        ...
        """
        # print(xt.size(), y.size(), alpha_t.size())
        # Evaluate the target
        target_log_prob, target_grad = target_log_prob_and_grad(xt)
        # Compute the log_prob
        log_prob = target_log_prob
        # print((torch.sum(torch.square(xt - alpha_t * y), dim=-1) / (1. - torch.square(alpha_t))).size())
        log_prob -= 0.5 * torch.sum(torch.square(xt - alpha_t * y), dim=-1, keepdim=True) / (1. - torch.square(alpha_t))
        log_prob = log_prob.view(-1)
        # log_prob -= 0.5 * torch.sum(torch.square(xt - torch.exp(-(T - t)) * y), dim=-1) / (1. - torch.exp(-2. * (T - t)))
        # Compute the gradient
        grad = target_grad
        grad += alpha_t * (xt - alpha_t * y) / (1. - torch.square(alpha_t))
        # grad += torch.exp(-t) * (torch.exp(-t) * y - xt) / (1. - torch.exp(-2. * t))
        # Return everything
        return log_prob, grad
    
    def posterior_importance_sampling(self, n_chains, x, alpha_t, target_log_prob, n_mc_samples=128):
    ...
    def score_estimation(self, x, alpha_t, target_log_prob, target_log_prob_and_grad, step_size, n_is_samples,
        ...
        y_langevin_start = self.posterior_importance_sampling(n_chains, x, alpha_t, target_log_prob, n_is_samples)
        ...
        def current_posterior_log_prob_and_grad(y): return self.posterior_log_prob_and_grad( y, x_reshaped,
                                                                                    alpha_t, target_log_prob_and_grad)
